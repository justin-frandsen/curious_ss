...
# Testing
t.test(bx_wide$validity_effect[bx_wide$phase == "testing"], mu = 0)
emmeans(aov_RT, pairwise ~ Validity | phase)
emmeans(aov_RT, pairwise ~ phase | Validity)

# analysis for target that never appeared in the training vs targets that did
...
#add condition info
FULL_SEARCH_PERIOD_fixation_summary <- FULL_SEARCH_PERIOD_fixation_summary %>% 
  left_join(all_bx_files %>% select(sub_num, run_num, trial_num, missing_target_flag, phase, valid0invalid1, rt, accuracy), 
...
critd_training <- FULL_SEARCH_PERIOD_fixation_summary %>%
  ...
  summarise(
    ...
    total_POST_SEARCH_PERIOD_fixations = sum(POST_SEARCH_PERIOD_total_fixations, na.rm = TRUE),
    total_POST_SEARCH_PERIOD_dur_CritDist = sum(POST_SEARCH_PERIOD_dur_CritDist, na.rm = TRUE),
    total_POST_SEARCH_PERIOD_fixation_on_critd = sum(POST_SEARCH_PERIOD_fixations_on_CritDist, na.rm = TRUE),
    .groups = "drop"
  ) 
...
#total_POST_SEARCH_PERIOD_fixations
#total_POST_SEARCH_PERIOD_dur_CritDist
#total_POST_SEARCH_PERIOD_fixation_on_critd

# Full model
dur_model1 <- lm(mean_target_first_fix ~ Validity.x,
             data = fixation_summary_for_regression)
dur_model2 <- lm(mean_target_first_fix ~ total_POST_SEARCH_PERIOD_fixations,
              data = fixation_summary_for_regression %>% filter(phase == "testing"))
dur_model3 <- lm(mean_target_first_fix ~ total_CritDist_z + Validity.x,
             data = fixation_summary_for_regression %>% filter(phase == "testing"))
dur_model4 <- lm(mean_target_first_fix ~ total_CritDist_z * Validity.x,
              data = fixation_summary_for_regression %>% filter(phase == "testing"))
...
cor.test(fixation_summary_for_regression$mean_target_first_fix, 
         fixation_summary_for_regression$total_CritDist_z)

# Full model
num_fix_model1 <- lm(mean_target_first_fix ~ Validity.x,
              data = fixation_summary_for_regression)
num_fix_model2 <- lm(mean_target_first_fix ~ total_POS,
              data = fixation_summary_for_regression %>% filter(phase == "testing"))
num_fix_model3 <- lm(mean_target_first_fix ~ total_number_fixation_on_critd + Validity.x,
              data = fixation_summary_for_regression %>% filter(phase == "testing"))
num_fix_model4 <- lm(mean_target_first_fix ~ total_number_fixation_on_critd * Validity.x,
              data = fixation_summary_for_regression %>% filter(phase == "testing"))

par(mfrow = c(2, 2))
plot(model3)
for_hist <- fixation_summary_for_regression %>% filter(phase == "testing")
hist(for_hist$total_number_fixations)

summary(num_fix_model1)
summary(num_fix_model2)
summary(num_fix_model3)
summary(num_fix_model4)

rt_model1 <- lm(meanRT ~ Validity.x,
                      data = fixation_summary_for_regression%>% filter(phase == "testing"))
rt_model2 <- lm(meanRT ~ total_number_fixation_on_critd,
                 data = fixation_summary_for_regression %>% filter(phase == "testing"))
rt_model3 <- lm(meanRT ~ total_number_fixation_on_critd + Validity.x,
                      data = fixation_summary_for_regression %>% filter(phase == "testing"))
rt_model4 <- lm(meanRT ~ total_number_fixation_on_critd * Validity.x,
                      data = fixation_summary_for_regression %>% filter(phase == "testing"))
...
lmer_model1 <- lmer(meanRT ~ Validity.x + (1 | sub_num),
               data = fixation_summary_for_regression %>% filter(phase == "testing"))
lmer_model2 <- lmer(mean_target_first_fix ~ total_number_fixation_on_critd + (1 | sub_num),
                    data = fixation_summary_for_regression %>% filter(phase == "testing"))
summary(lmer_model1)
summary(lmer_model2)


ggplot(fixation_summary_for_regression, aes(x = total_number_fixation_on_critd)) +
  geom_histogram(bins = 10, fill = "steelblue") +
  facet_wrap(~Validity.x)

ggplot(fixation_summary_for_regression, aes(x = mean_target_first_fix)) +
